...
function processEEGDataToCSV_Regional(participant_numbers, output_csv_path)
    ...
    % Call function epochAndCalculatePSD, then display the values 
        region_powers = epochAndCalculatePSD(EEG);
        disp(region_powers);

  
    %   Extract and format the PSD data for CSV
         psd_data = {participant_numbers{i}};
       for r = regions
            for b = bands
              psd_data{end+1} = region_powers.(r{1}).(b{1});
            end
        end
